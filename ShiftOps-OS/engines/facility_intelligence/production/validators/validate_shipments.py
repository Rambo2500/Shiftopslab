...
def validate_shipments(df):
    required_cols = ["sales date", "item code", "qty shipped"]
    ...
    try:
        ...
        units_col = df.columns[cols_found.index("qty shipped")]
    ...
    item_code_col = df.columns[cols_found.index("item code")]
    skus_in_file = set(df[item_code_col].astype(str).str.strip().unique())
    ...
    return {"valid": True, "message": "WMS Shipments file validated successfully."}
