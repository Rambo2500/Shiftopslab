...
class PlanningKernel:
    ...
    def _expand_intent(self, user_request: str, complexity: str = "MEDIUM", domain_data: Dict[str, Any] = None) -> str:
        ...
        if complexity == "LOW":
            prompt = f"""
            Expand this software request into a minimalist, lightweight system requirement.
            User Request: "{user_request}"
            {domain_context}
            
            Focus on simplicity and low-cost implementation. 
            Prefer single-file scripts, local data tracking (like Excel/CSV), and direct interfaces.
            Avoid microservices, distributed systems, or complex message queues. 
            The solution should be "one man can maintain this in a weekend".
            """
        else:
            patterns = ["Event-Driven Microservices", "Layered Monolith", "Service Mesh", "Serverless Functions", "Reactive Streams"]
            selected_pattern = random.choice(patterns)
            design_seed = random.randint(1000, 9999)
            
            prompt = f"""
            Expand the following software request into a professional, enterprise-grade system requirement description.
            
            User Request: "{user_request}"
            Architectural Focus: {selected_pattern}
            Design Entropy Seed: {design_seed}
            {domain_context}
            
            Think of global scale, high-availability, and modern UX. 
            Focus on specific technical components and data flows.
